            ...
            try
            {
                ...
                if (context.PrimaryEntityName.ToLower() == "xom_guyanalearningapplicationaction" && (context.MessageName.ToLower() == "create"))
                {
                    ...
                    if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                    {
                        ...
                        {
                            tracingService.Trace("Action Notes description Contains Data.");
                            ...
                            tracingService.Trace("New annotation created for Action with ActionNotes Value.");
                        }
                        else // Action Description Doesn't have Value
                        {
                            tracingService.Trace("Action Notes Description does not Contain Data, So no Annotation is Created.");
                        }
                    }
                }
                #endregion
                    ...
                    if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                    {
                        ...
                        if (latestNote == null)// This Condition Checks If no Existing Annotation is Associated with Action.
                        {
                            ...
                            if (targetEntity.GetAttributeValue<string>("llm_notes") != "N/A") 
                            {
                                ...
                                annotation["subject"] = "Note 1 on Update of Action";
                                ...
                                tracingService.Trace("New Notes Created on Update of Actions NoteDescription");
                            }
                            else// because the Action Description is N/A it should not create a Annotation
                            {
                                tracingService.Trace("Action description is N/A,so no new annotation is Created.");
                            }
                        }
                        else
                        {
                            string AnnotationNoteText = StripHTML(latestNote.GetAttributeValue<string>("notetext"));
                                ...
                                if (notesDescription != AnnotationNoteText)
                                {
                                    ...
                                    notes["objectid"] = new EntityReference("account", targetEntity.Id);
